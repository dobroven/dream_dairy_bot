...
└── prompts/
    ...
    ├── compiled_books.md    # Автоматически скомпилированный конспект книги
...
2. **Компиляция.** При первом нажатии на кнопку «🗺 Карта снов» бот отправляет книгу в Gemini Flash, который извлекает ключевые концепции, техники и методы поиска паттернов. Результат сохраняется в `prompts/compiled_books.md`.
